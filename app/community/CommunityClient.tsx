...
function Avatar({ name, size = 36, url }: { name: string; size?: number; url?: string | null }) {
  ...
  return (
    <div
      ...
      style={{ width: size, height: size, backgroundColor: `hsl(${hue},45%,32%)`, fontSize: size * 0.35 }}
      aria-hidden="true"
    >
      {initials}
    </div>
...
// ── Main Page ─────────────────────────────────────────────────────────────────
export default function CommunityPage({ initialPosts = [] }: { initialPosts?: Post[] }) {
  ...
  return (
    <main className="min-h-screen" id="main-content" style={{ backgroundColor: '#F7F5F0' }}>
      ...
      <div className="relative py-16 px-6 text-center"
        ...
        {/* Niko + Picasso sitting inside the hero, bottom corners */}
        <div
          aria-hidden="true"
        ...
        <div
          ...
          style={{ zIndex: 10 }}
        >
          <img src="/picasso/picasso-1-nobg.png" alt="" width={160}
